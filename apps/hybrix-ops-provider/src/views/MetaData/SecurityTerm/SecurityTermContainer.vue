    ...
    <div class="container">
      ...
      <h-card class="grid__card">
        <h-data-table
          ref="gridRef"
          :datas="SecurityTerm"
          :column-list="SecurityTermColumns"
...
import {
  ...
  updateSecurityTerm,
  uploadSecurityTerm,
} from '@/api/modules/provider/security-term';

const gridRef = ref();
const gridSettings = ref(settings);

const SecurityTerm = ref<any[]>([]);
const SecurityTermColumns = ref<ColDef[]>([]);
...
const uploadFile = async () => {
  ...
  const uploadRes = await uploadSecurityTerm(formData);
...
const cellChanged = async (e: CellValueChangedEvent | ICellRendererParams) => {
  if (!e.column) return;
  const updateSecurityTermDto: any = {};
  updateSecurityTermDto['securityTerm'] = e.data['securityTerm'];
  updateSecurityTermDto[e.column.getColId()] = e.value;

  await updateSecurityTerm(updateSecurityTermDto);
};
...
const fetch = async () => {
  ...
  const res = await findSecurityTerm({ query });
  SecurityTerm.value = res.data.value.data;
};

const setColumns = async () => {
  SecurityTermColumns.value = columnDefs;
  for (const column of SecurityTermColumns.value) {
    if (column.field === 'securityTerm') {
