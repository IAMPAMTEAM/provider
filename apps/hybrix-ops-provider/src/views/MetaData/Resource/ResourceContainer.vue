  ...
  <main class="main">
    ...
    <div class="container">
      ...
      <h-card class="grid__card">
        <h-data-table
          ref="gridRef"
          :datas="awsResourceType"
          :column-list="awsResourceTypeColumns"
...
import {
  findAwsResourceType,
  updateAwsResourceType,
  uploadAwsResourceType,
} from '@/api/modules/provider/aws-resourcetype';
import type { ColDef } from 'ag-grid-community';

const gridRef = ref();
const gridSettings = ref(settings);

const awsResourceType = ref<any[]>([]);
const awsResourceTypeColumns = ref<ColDef[]>([]);
...
const uploadFile = async () => {
  ...
  const uploadRes = await uploadAwsResourceType(formData);
...
const cellChanged = async (e: CellValueChangedEvent | ICellRendererParams) => {
  if (!e.column) return;
  const updateAwsResourceTypeDto: any = {};
  updateAwsResourceTypeDto['awsResourceTypeName'] = e.data['awsResourceTypeName'];
  updateAwsResourceTypeDto[e.column.getColId()] = e.value;

  await updateAwsResourceType(updateAwsResourceTypeDto);
};
...
const setColumns = async () => {
  ...
  for (const column of awsResourceTypeColumns.value) {
    if (column.field === 'awsResourceTypeName') {
