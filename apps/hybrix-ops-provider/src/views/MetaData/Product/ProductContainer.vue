...
import {
  ...
  updateAwsProduct,
  uploadAwsProduct,
} from '@/api/modules/provider/aws-product';
import type { ColDef } from 'ag-grid-community';

const gridRef = ref();
const gridSettings = ref(settings);

const awsProducts = ref<any[]>([]);
const awsProductColumns = ref<ColDef[]>([]);
...
const uploadFile = async () => {
  ...
  const uploadRes = await uploadAwsProduct(formData);
...
const cellChanged = async (e: CellValueChangedEvent | ICellRendererParams) => {
  if (!e.column) return;
  const updateAwsProductDto: any = {};
  updateAwsProductDto['awsProductCode'] = e.data['awsProductCode'];
  updateAwsProductDto[e.column.getColId()] = e.value;

  await updateAwsProduct(updateAwsProductDto);
};
...
const fetch = async () => {
  const { data } = await findAwsProduct();
  awsProducts.value = JSON.parse(data.value.body);
};
const setColumns = async () => {
  awsProductColumns.value = columnDefs;
  for (const column of awsProductColumns.value) {
    if (column.field === 'awsProductCode') {
