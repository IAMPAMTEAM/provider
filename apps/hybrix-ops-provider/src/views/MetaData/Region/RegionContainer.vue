...
const awsRegion = ref<any[]>([]);
const awsRegionColumns = ref<ColDef[]>([]);
...
const uploadFile = async () => {
  ...
  const uploadRes = await uploadAwsRegion(formData);
...
const cellChanged = async (e: CellValueChangedEvent | ICellRendererParams) => {
  if (!e.column) return;
  const updateAwsRegionDto: any = {};
  updateAwsRegionDto['awsRegionCode'] = e.data['awsRegionCode'];
  updateAwsRegionDto[e.column.getColId()] = e.value;

  await updateAwsRegion(updateAwsRegionDto);
};
...
const setColumns = async () => {
  ...
  for (const column of awsRegionColumns.value) {
    if (column.field === 'awsRegionCode') {
